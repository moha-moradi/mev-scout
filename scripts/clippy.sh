...
#
# Usage: bash scripts/clippy.sh   (or ./scripts/clippy.sh after chmod +x)

set -euo pipefail

echo "==> cargo clippy (dead-code errors on lib + bins)"
cargo clippy --workspace --lib --bins -- -W dead_code -D dead_code

echo "==> clippy guardrail passed: no dead code in lib/bins"
