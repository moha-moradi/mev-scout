...
fn main() -> anyhow::Result<()> {
    ...
    let (refunded, floor, spent) = match &result {
        ...
        | ExecutionResult::Halt { gas, .. } => {
            (gas.inner_refunded(), gas.floor_gas(), gas.total_gas_spent())
        }
    };
    println!(
    ...
    sorted.sort_by_key(|(_op, cost, _remaining)| std::cmp::Reverse(*cost));
    println!("\n=== top-20 expensive ops ===");
    for (op, cost, _remaining) in sorted.iter().take(20) {
        println!("  {} (0x{:02x}) cost={}", op_name(*op), op, cost);
    }

    // all SLOAD/SSTORE ops
    let ss_ops: Vec<_> = ops
        .iter()
        .filter(|(op, _, _)| *op == 0x54 || *op == 0x55)
        .collect();
    println!("\n=== SLOAD/SSTORE details ({} total) ===", ss_ops.len());
    for (op, cost, _remaining) in ss_ops.iter().take(40) {
