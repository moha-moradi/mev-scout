...
/// Test RPC URL for live-RPC integration tests.
///
/// Opt-in gating (mirrors `cli/tests/cli_e2e.rs`): returns the `RPC_URL` env
/// var only when `MEV_SCOUT_E2E=1` is set as well; otherwise `None` so callers
/// skip outright. There is deliberately no fallback to the repo
/// `mev-scout.toml`, so plain `cargo test` runs never do incidental live
/// network I/O against the configured providers.
