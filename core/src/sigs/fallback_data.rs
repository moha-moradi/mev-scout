//! Static fallback signature data for `build_fallback_db`.
//!
//! Data-only module: the two tables below were extracted from
//! `downloader.rs` so the builder keeps just schema + insert logic.

/// Function selector hex (no 0x) -> canonical signature.
pub(super) static FALLBACK_METHODS: &[(&str, &str)] = &[
...
/// Event topic hex (no 0x) -> canonical signature.
pub(super) static FALLBACK_EVENTS: &[(&str, &str)] = &[
    // ERC20 / Generic
    (
