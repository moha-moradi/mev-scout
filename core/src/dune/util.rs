...
pub fn chain_timing(chain: &str) -> ChainTimingParams {
    let lag = (DUNE_LAG_DAYS * 24 * 3600) as i64;
    match chain.to_lowercase().as_str() {
        "ethereum" => ChainTimingParams { genesis_ts: 1438269988, secs_per_block: 12.0, blocks_per_day: 7200, dune_lag_secs: lag, anchor_block: 0, anchor_ts: 0 },
        ...
        "bsc" => ChainTimingParams { genesis_ts: 1597734000, secs_per_block: 3.0, blocks_per_day: 28800, dune_lag_secs: lag, anchor_block: 0, anchor_ts: 0 },
        "avalanche" | "avalanche_c" => ChainTimingParams { genesis_ts: 1624402800, secs_per_block: 2.0, blocks_per_day: 43200, dune_lag_secs: lag, anchor_block: 0, anchor_ts: 0 },
        "arbitrum" => ChainTimingParams { genesis_ts: 1630812600, secs_per_block: 0.26, blocks_per_day: 330000, dune_lag_secs: lag, anchor_block: 0, anchor_ts: 0 },
        "base" => ChainTimingParams { genesis_ts: 1686787200, secs_per_block: 2.0, blocks_per_day: 43200, dune_lag_secs: lag, anchor_block: 0, anchor_ts: 0 },
        "optimism" => ChainTimingParams { genesis_ts: 1631808000, secs_per_block: 2.0, blocks_per_day: 43200, dune_lag_secs: lag, anchor_block: 0, anchor_ts: 0 },
        _ => ChainTimingParams { genesis_ts: 1609459200, secs_per_block: 12.0, blocks_per_day: 7200, dune_lag_secs: lag, anchor_block: 0, anchor_ts: 0 },
    }
}
...
/// Days to subtract from the linear block-time estimate before deriving the
/// `block_month` lower bound. The linear model drifts late for windows further
/// in the past (e.g. ~5 days at 90 days back on Polygon), which would silently
/// drop the start of the target window from `block_month` pruning. A
/// conservatively early bound only scans extra data and never excludes rows.
const MONTH_MIN_SAFETY_DAYS: i64 = 15;

pub fn approx_block_month_min(block_number: u64, chain: &str) -> String {
    let approx_ts = block_timestamp_secs(block_number, chain)
        .saturating_sub(MONTH_MIN_SAFETY_DAYS * 86400);
    let naive = chrono::DateTime::from_timestamp(approx_ts, 0)
        .unwrap_or_default();
    naive.format("%Y-%m-01").to_string()
}
...
mod tests {
    ...
    #[test]
    fn polygon_month_min_matches_verified_block() {
    ...
    #[test]
    fn polygon_month_min_is_conservatively_early() {
        // Block 86,236,778 = 2026-05-01 00:00 UTC (RPC-verified). The linear
        // model lands ~5 days late (2026-05-06); the bound must stay at or
        // before the true month start so block_month pruning never drops May.
        assert!(approx_block_month_min(86_236_778, "polygon") < "2026-05-01".to_string());
        // A recent-window block must also never round past its own month.
        assert!(approx_block_month_min(91_000_000, "polygon") <= "2026-07-01".to_string());
    }
}
