...
mod tests {
    ...
    fn test_validate_conflicting_flags() {
        ...
        let err = validate_and_resolve(&cfg).unwrap_err();
        assert!(err.to_string().contains("cannot be used together"));
    }

    #[test]
    fn test_validate_from_without_to() {
        let cfg = Config {
            chain: "polygon".into(),
            from_block: Some(100),
            ..Config::default()
    ...
    #[test]
    fn test_validate_to_less_than_from() {
        let cfg = Config {
            chain: "polygon".into(),
            from_block: Some(200),
            to_block: Some(100),
    ...
    #[test]
    fn test_validate_days_out_of_range() {
        let cfg = Config {
            chain: "polygon".into(),
            days: Some(400),
            ..Config::default()
        ...
        let err = validate_and_resolve(&cfg).unwrap_err();
        assert!(err.to_string().contains("days must be between 1 and 365"));
    }
    ...
    fn test_validate_block_zero() {
        ...
        let err = validate_and_resolve(&cfg).unwrap_err();
        assert!(err.to_string().contains("block must be > 0"));
    }

    #[test]
    fn test_validate_no_range_flag() {
        let cfg = Config {
            chain: "polygon".into(),
            ..Config::default()
        };
        let err = validate_and_resolve(&cfg).unwrap_err();
        assert!(err.to_string().contains("no block range specified"));
    }

    #[test]
    fn test_validate_unknown_chain() {
        let cfg = Config {
    ...
    #[test]
    fn test_validate_bad_rpc_url_scheme() {
        let cfg = Config {
            chain: "polygon".into(),
            block: Some(100),
            rpc_url: Some("ftp://rpc.example.com".into()),
            ..Config::default()
        };
        let err = validate_and_resolve(&cfg).unwrap_err();
    ...
    #[test]
    fn test_validate_replay_rejects_days() {
        let cfg = Config {
            chain: "polygon".into(),
            days: Some(7),
            ..Config::default()
        };
        let err = validate_replay(&cfg).unwrap_err();
        assert!(err.to_string().contains("days is not supported by the replay"));
    }

    #[test]
    fn test_validate_replay_rejects_blocks() {
        let cfg = Config {
            chain: "polygon".into(),
            blocks: Some(100),
            ..Config::default()
        ...
        let err = validate_replay(&cfg).unwrap_err();
        assert!(err.to_string().contains("blocks is not supported by the replay"));
    }

    #[test]
    fn test_validate_replay_rejects_range() {
        let cfg = Config {
            chain: "polygon".into(),
            from_block: Some(100),
            to_block: Some(200),
        ...
        let err = validate_replay(&cfg).unwrap_err();
        assert!(err.to_string().contains("from-block/--to-block is not supported by the replay"));
    }

    #[test]
    fn test_validate_replay_requires_block() {
        let cfg = Config {
            chain: "polygon".into(),
            ..Config::default()
        };
        let err = validate_replay(&cfg).unwrap_err();
        assert!(err.to_string().contains("--block is required for the replay"));
    }

    #[test]
    fn test_validate_replay_valid_single_block() {
        let cfg = Config {
            chain: "polygon".into(),
            block: Some(50_000_000),
            ..Config::default()
        };
        let result = validate_replay(&cfg).unwrap();
        assert_eq!(result.0, ChainName::Polygon);
    }

    #[test]
    fn test_validate_blocks_less_than_one() {
        let cfg = Config {
            chain: "polygon".into(),
            blocks: Some(0),
            ..Config::default()
        ...
        let err = validate_and_resolve(&cfg).unwrap_err();
        assert!(err.to_string().contains("blocks must be >= 1"));
    }
}
