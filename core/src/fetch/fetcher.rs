...
pub struct FetchSummary {
    ...
    pub phase_db_ms: f64,
    pub phase_distribute_ms: f64,
    pub phase_integrity_ms: f64,
...
impl Fetcher {
    ...
    /// Fetch all blocks in the range using shard-pinned distribution.
    ///
    ...
    pub async fn fetch_range<F: Fn() + Sync>(
        ...
        let phase_integrity_ms = t_integrity.elapsed().as_secs_f64() * 1000.0;

        let t_flush = Instant::now();
        self.cache.flush()?;
        let phase_flush_ms = t_flush.elapsed().as_secs_f64() * 1000.0;
        ...
        final_summary.phase_db_ms = phase_db_ms;
        final_summary.phase_distribute_ms = phase_distribute_ms;
        final_summary.phase_integrity_ms = phase_integrity_ms;
        final_summary.phase_flush_ms = phase_flush_ms;
        if let Ok(t) = self.timing.lock() {
            final_summary.timing = t.clone();
        }
        tracing::info!(
            "fetch_range: {} blocks ({}/{}) shards={} DB={:.1}ms distribute={:.1}ms integrity={:.1}ms flush={:.1}ms total={:.1}s | {}",
            final_summary.total_blocks, final_summary.fetched, final_summary.cached,
            shards.len(), phase_db_ms, phase_distribute_ms, phase_integrity_ms, phase_flush_ms,
            final_summary.elapsed_secs,
    ...
    async fn fetch_contiguous_range<F: Fn() + Sync>(
        ...
        if cap <= 1 {
            return self
                .fetch_contiguous_range_sequential(start, end, timing, progress, provider_idx)
                .await;
        }
        ...
        for block_num in start..=end {
            ...
            tasks.push(async move {
                ...
                let t2 = Instant::now();
                write_block_data(&sig, &cache, &write_buf, block_num, block, txs, receipts)?;
    ...
    pub async fn fetch_relevant<F: Fn() + Sync>(
        ...
        let missing_after = self.cache.check_integrity_range(&sorted)?;
...
fn resolve_tx_sigs(txs: &[TxData], resolver: &SignatureResolver) -> Vec<TxSigEntry> {
    txs.iter()
        .map(|tx| match extract_selector(&tx.input) {
            Some(sel) => {
