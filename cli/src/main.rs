...
use mev_scout_core::cli::{Cli, Command, RunArgs, FetchArgs, ReportArgs, ReplayArgs, DiscoverArgs, FactCheckArgs, LiveArgs};
...
use mev_scout_core::types::{ChainName, GasConfig, GasModel, OutputFormat, PriceOracleMode, Strategy};
...
async fn cmd_fetch(config: &Config, args: &FetchArgs) -> anyhow::Result<()> {
...
async fn cmd_report(_config: &Config, args: &ReportArgs) -> anyhow::Result<()> {
...
async fn cmd_discover(config: &Config, args: &DiscoverArgs) -> anyhow::Result<()> {
    let chain_name: ChainName = match args.chain_args.chain.parse() {
        Ok(c) => c,
...
async fn cmd_factcheck(config: &Config, args: &FactCheckArgs) -> anyhow::Result<()> {
...
async fn cmd_live(config: &Config, args: &LiveArgs) -> anyhow::Result<()> {
    let chain_name: ChainName = match args.chain_args.chain.parse() {
        Ok(c) => c,
        Err(e) => anyhow::bail!("Error: {e}"),
    };
...
async fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Command::Report(args) => cmd_report(&config, args).await,
        Command::Config => cmd_config(&config).await,
        Command::Replay(args) => cmd_replay(&config, args).await,
        Command::Discover(args) => cmd_discover(&config, args).await,
        Command::FactCheck(args) => cmd_factcheck(&config, args).await,
        Command::Live(args) => cmd_live(&config, args).await,
    }
}